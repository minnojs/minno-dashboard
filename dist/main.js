(function () {
	        ...
	        m('.row',[
	            m('.col-md-6',[
	                ...
	                m('.row',
	                    m('.col-md-7',
	                        m('.btn-group btn-group-toggle', [
	                            m('button[type=button].btn btn-outline-secondary',{disabled:!ctrl.fields.newStimulus().length, id:'addWord', onclick: function (e) { return ctrl.addStimulus(e); }},[
	                                m('i.fa.fa-plus'), 'Add Word'
	                            ]),
	                            m('button[type=button].btn btn-outline-secondary', {disabled:!ctrl.fields.newStimulus().length, id:'addImage', onclick: function (e) { return ctrl.addStimulus(e); }},[
	            ...
	            !ctrl.fields.startStimulus() ? '' :
	                m('.col-md-6', [
	                    ...
	                    m('.row',
	                        m('.col-md-7',
	                            m('.btn-group btn-group-toggle', [
	                                m('button[type=button].btn btn-outline-secondary',{disabled:!ctrl.fields.newStartStimulus().length, id:'addWord', onclick: function (e) { return ctrl.addStimulus(e,true); }},[
	                                    m('i.fa.fa-plus'), 'Add Word'
	                                ]),
	                                m('button[type=button].btn btn-outline-secondary', {disabled:!ctrl.fields.newStartStimulus().length, id:'addImage', onclick: function (e) { return ctrl.addStimulus(e,true); }},[
	                                    m('i.fa.fa-plus'), 'Add Image'
	                                ])
	                            ])
	                        )
	...
	function controller$8(object, settings, stimuliList){
	    var element = settings[object.key];
	    var fields = {
	        newStimulus : m.prop(''),
	        elementType: m.prop(object.key.includes('attribute') ? 'Attribute' : 'Category'),
	...
	function view$5(ctrl, settings, defaultSettings, clearElement, subTabs, taskType, currTab) {
	    return m('div', [
	        taskType === 'BIAT' ?
	            ...
	            : currTab === 'primeStimulusCSS' ? //in EP there is additional subtab called Prime Design, it needs different component.
	                ...
	                : taskType === 'EP' ?
	                    m.component(elementComponent$2, {key:currTab}, settings, defaultSettings[currTab].mediaArray)
	...
	var save$1 = function (study_type, study_id, file_id, content) { return fetchJson(url$3(study_type, study_id, file_id), {
	    method: 'put',
	    body: {content: JSON.stringify(content)}
	}); };

	var saveToJS = function (study_type, study_id, file_id, content) { return fetchJson(url$3(study_type, study_id, file_id), {
	    method: 'put',
	...
	function controller(ref, external){
	    ...
	    function do_save(){
	        ...
	        save$1('iat', studyId, fileId, ctrl.settings)
	            .then (function () { return saveToJS('iat', studyId, jsFileId, toString(ctrl.settings, ctrl.external)); })
	            .then(ctrl.study.get())
	...
	function controller$11(ref, external){
	    ...
	    function do_save(){
	        ...
	        var fileId = m.route.param('fileId');
	        var jsFileId =  fileId.split('.')[0]+'.js';
	        save$1('biat', studyId, fileId, ctrl.settings)
	            .then (function () { return saveToJS('biat', studyId, jsFileId, toString$1(ctrl.settings, ctrl.external)); })
	            .then(ctrl.study.get())
	...
	function controller$16(ref, external){
	    ...
	    function do_save(){
	        ...
	        var fileId = m.route.param('fileId');
	        var jsFileId =  fileId.split('.')[0]+'.js';
	        save$1('spf', studyId, fileId, ctrl.settings)
	...
	function updateMediaSettings$1(settings, input){
	...
	function updateSettings$7(settings, input) {
	    ...
	    settings = updateMediaSettings$1(settings, input);
	    return settings;
	...
	function controller$19(ref, external){
	    ...
	    function do_save(){
	        ...
	        var fileId = m.route.param('fileId');
	        var jsFileId =  fileId.split('.')[0]+'.js';
	        save$1('stiat', studyId, fileId, ctrl.settings)
	            .then (function () { return saveToJS('stiat', studyId, jsFileId, toString$3(ctrl.settings, ctrl.external)); })
	            .then(ctrl.study.get())
	...
	function updateMediaSettings$3(settings, input){
	    //update attributes to be compatible to IAT so that elementComponent can be used.
	...
	function updateSettings$9(settings, input) {
	    ...
	    settings = updateMediaSettings$3(settings, input);
	    return settings;
	}
	...
	function controller$23(ref, external){
	    ...
	    function do_save(){
	        ...
	        var fileId = m.route.param('fileId');
	        var jsFileId =  fileId.split('.')[0]+'.js';
	        save$1('ep', studyId, fileId, ctrl.settings)
	            .then (function () { return saveToJS('ep', studyId, jsFileId, toString$4(ctrl.settings, ctrl.external)); })
	            .then(ctrl.study.get())
