(function () {
                ...
                var baseUrl$2 = '/dashboard/dashboard/studies';


                function get_prop_url(study_id)
                {	
                    return ("" + baseUrl$2 + "/" + (encodeURIComponent(study_id)) + "/prop");
                }

                function deploy_url(study_id)
                {	
                    return ("" + baseUrl$2 + "/" + (encodeURIComponent(study_id)) + "/deploy");
                }
                ...
                var deploy = function (study_id, folder_location,researcher_email, researcher_name, target_number, approved_by_a_reviewer, experiment_file, launch_confirmation, comments) { return fetchJson(deploy_url(study_id), {
                	method: 'post',
                	body: {folder_location: folder_location,researcher_email: researcher_email, researcher_name: researcher_name, target_number: target_number, approved_by_a_reviewer: approved_by_a_reviewer, experiment_file: experiment_file, launch_confirmation: launch_confirmation, comments: comments}
                }); };

                var study_removal = function (study_id, study_name, researcher_email, researcher_name, completed_n, comments) { return fetchJson(deploy_url(study_id), {
                	method: 'delete',
                	body: {study_name: study_name, researcher_email: researcher_email, researcher_name: researcher_name, completed_n: completed_n, comments: comments}
                }); };

                var Study_change_request = function (study_id, file_names, researcher_email, researcher_name, target_sessions, study_showfiles_link, status, comments) { return fetchJson(deploy_url(study_id), {
                	method: 'put',
                	body: {file_names: file_names, researcher_email: researcher_email, researcher_name: researcher_name, target_sessions: target_sessions, study_showfiles_link: study_showfiles_link, status: status, comments: comments}
                }); };
                ...
                var deployComponent = {
                    controller: function controller(){
                        ...
                        get_study_prop(m.route.param('studyId'))
                            .then(function ( response ) {
                                ...
                                messages$1.alert({header:'Select expt file', 
                                        content:response.experiment_file.map(function (experiment_file) {
                                        return radioInput({description: experiment_file.file_name, prop: ctrl.experiment_file, form: form, name: 'experiment_file_name'});
                                    })});
                                })
                    ...
                    view: function view(ref){
                        ...
                        return  m('.deploy.container', [
                                m('h1', 'Deploy'),
                                textInput({label:'Researcher name',  placeholder: 'Researcher name', prop: ctrl.researcher_name, form: form, required:true}),
                                textInput({label:'Researcher email address',  placeholder: 'Researcher email address', prop: ctrl.researcher_email, form: form, required:true}),
                                textInput({label:'Study folder location',  placeholder: 'Study folder location', prop: ctrl.folder_location, form: form, required:true}),
                                textInput({label:'Name of experiment file',  placeholder: 'Name of experiment file', prop: ctrl.experiment_file, form: form, required:true}),
                                textInput({help: 'For private studies (not in the Project Implicit research pool), enter n/a', label:'Target number of completed study sessions',  placeholder: 'Target number of completed study sessions', prop: ctrl.target_number, form: form, required:true}),
                                ...
                                checkboxInput({description: 'Did you make sure your study-id starts with your user name', prop: ctrl.valid_study_name, form: form, required:true}),                
                                checkboxInput({description: m('span', ['This study has been approved by the appropriate IRB ', m('span.text-danger', '*')]), prop: ctrl.approved_by_irb, form: form, required:true}),                
                                ...
                                checkboxInput({description: m('span', ['My study folder includes ZERO files that aren\'t necessary for the study (e.g., word documents, older versions of files, items that were dropped from the final version) ', m('span.text-danger', '*')]), prop: ctrl.zero_unnecessary_files, form: form, required:true}),                
                                
                                m('p', m('span', ['Study approved by a *User Experience* Reviewer (Calvin Lai) ', m('span.text-danger', '*')])),
                                radioInput({description: 'Yes', prop: ctrl.approved_by_a_reviewer, form: form, name: 'approved_by_a_reviewer'}),                
                                radioInput({description: 'No, this study is not for the Project Implicit pool.', prop: ctrl.approved_by_a_reviewer, form: form, name: 'approved_by_a_reviewer'}),
                                ...
                                radioInput({description: 'Yes', prop: ctrl.launch_confirmation, form: form, name: 'launch_confirmation'}),                
                                radioInput({description: 'No,this study is mine.', prop: ctrl.launch_confirmation, form: form, name: 'launch_confirmation'}),                
                                checkboxInput({description: 'Did you use a realstart and lastpage task?', prop: ctrl.realstart, form: form, required:true}),                
                                textInput({isArea: true, label: m('span', 'Additional comments'),  placeholder: 'Additional comments', prop: ctrl.comments, form: form}),
                ...
                //                    ctrl.folder_location(response.folder);
                    ...
                    view: function view(ref){
                        ...
                        return  m('.StudyRemoval.container', [
                                m('h1', 'Study Removal'),
                                textInput({label: m('span', ['Researcher name', m('span.text-danger', ' *')]),  placeholder: 'Researcher name', prop: ctrl.researcher_name, form: form, required:true}),
                                textInput({label: m('span', ['Researcher email address', m('span.text-danger', ' *')]),  placeholder: 'esearcher email address', prop: ctrl.researcher_email, form: form, required:true}),
                                textInput({label: m('span', ['Study name', m('span.text-danger', ' *')]), help: 'This is the name you submitted to the RDE (e.g., colinsmith.elmcogload) ',  placeholder: 'Study name', prop: ctrl.study_name, form: form, required:true}),
                ...
                //            m('span', ['you can use the following link ',m('a', {href:'https://app-prod-03.implicit.harvard.edu/implicit/research/pitracker/PITracking.html#3'})])
                                textInput({isArea: true, label: m('span', 'Additional comments'), help: '(e.g., anything unusual about the data collection, consistent participant comments, etc.)',  placeholder: 'Additional comments', prop: ctrl.comments, form: form}),
                ...
                //                    ctrl.researcher_email(response.researcher_email);
                    ...
                    view: function view(ref){
                        ...
                        return  m('.StudyChangeRequest.container', [
                                ...
                                textInput({label: m('span', ['Researcher name', m('span.text-danger', ' *')]),  placeholder: 'Researcher name', prop: ctrl.researcher_name, form: form, required:true}),
                                textInput({label: m('span', ['Researcher email address', m('span.text-danger', ' *')]),  placeholder: 'esearcher email address', prop: ctrl.researcher_email, form: form, required:true}),
                                textInput({label: m('span', ['Target number of additional sessions (In addition to the sessions completed so far)', m('span.text-danger', ' *')]),  placeholder: 'Target number of additional sessions', prop: ctrl.target_sessions, form: form, required:true}),

                                m('p', 'What\'s the current status of your study?*'),
                                radioInput({description: 'Currently collecting data and does not need to be unpaused', prop: ctrl.status, form: form, name: 'status'}),                
                                radioInput({description: 'Manually paused and needs to be unpaused', prop: ctrl.status, form: form, name: 'status'}),                
                                radioInput({description: 'Auto-paused due to low completion rates or meeting target N.', prop: ctrl.status, form: form, name: 'status'}),                

                                textInput({label:  m('span', ['Study showfiles link', m('span.text-danger', ' *')]), placeholder: 'Study showfiles link', help: m('span', 'For example: http://app-prod-03.implicit.harvard.edu/implicit/showfiles.jsp?user=rhershman&study=examplestudy'),prop: ctrl.study_showfiles_link, form: form}),,
                                
                                textInput({isArea: true, label: m('span', ['Change Request', m('span.text-danger', ' *')]), help: 'List all file names involved in the change request. Specify for each file whether file is being updated or added to production.)',  placeholder: 'Change Request', prop: ctrl.file_names, form: form, required:true}),
                                textInput({isArea: true, label: m('span', 'Additional comments'),  placeholder: 'Additional comments', prop: ctrl.comments, form: form}),
